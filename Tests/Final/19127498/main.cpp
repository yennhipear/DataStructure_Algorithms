...
void takeL_G (ifstream &fi, int &L, int &G)
{
    string line1;      getline(fi, line1);
...
void LevelOrder(NODE* &pRoot, int L, int G)
{
    NODE *temp = pRoot;
    ...
    {
        NODE *cur = Q.front();
        if ((Level(temp, cur) + 1 == L) && (cur->key % G == 0))
            { cout << cur->key << " "; }
...
NODE *FindNode (NODE *&root)
{
    ...
    NODE *temp = FindNode(root->p_left);
    if (temp != NULL)   { return temp; }

    if ((root->p_left != NULL) && (root->p_right != NULL) && (countNode(root->p_left) - countNode(root->p_right) > 0))    { return root; }
